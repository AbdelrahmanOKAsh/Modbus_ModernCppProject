...
ModbusResponse::ModbusResponse(uint8_t slaveId, utils::MBFunctionCode functionCode,uint16_t address, uint16_t registersNumber,std::vector<ModbusCell> values)
...
std::string ModbusResponse::toString() 
{
    std::stringstream result;

    result << utils::mbFunctionToStr(_functionCode) << ", from slave " + std::to_string(_slaveID);

    if (functionType() != utils::WriteSingle)
    {
        result << ", starting from address " + std::to_string(_address) << ", on " + std::to_string(_registersNumber) + " registers";
...
std::vector<uint8_t> ModbusResponse::toRaw()
{
