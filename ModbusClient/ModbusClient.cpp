...
ModbusRequest::ModbusRequest(uint8_t slaveId, utils::MBFunctionCode functionCode,uint16_t address, uint16_t registersNumber,std::vector<ModbusCell> values) :
...
std::vector<uint8_t> ModbusRequest::toRaw()
{
    ...
    {
        result.push_back((_registersNumber / 8) + (_registersNumber % 8 == 0 ? 0 : 1));
    }
